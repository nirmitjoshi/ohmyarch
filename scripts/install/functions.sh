...
# $2 variable value
set_pref() {
    if grep -Eq "^${1}.*" $PREF_FILE; then # checks if preference exist
        sed -i -e "/^${1}.*/d" $PREF_FILE # deletes preference if it exists
    fi
    echo "${1}=${2}" >>$PREF_FILE # adds preference
}

# Function to get a single character from user
getch() {
    local char

    # Disable terminal buffering (read without waiting for Enter)
    stty -echo
    stty raw
...
get_username() {
    ...
    # Check if the username starts with a hyphen
    if [[ "${username:0:1}" == "-" ]]; then
        echo "Invalid: Username cannot start with a hyphen."
        get_username  # Recursive call
    fi

    # Check if the username is reserved or restricted
    case "$username" in
...
# Functions to input and validate passwords
...
get_timezone() {
  ...
  case ${selection} in
      ...
      *) echo "Wrong option. Try again";get_timezone;;
  esac
}
...
get_disk() {
    ...
    case ${confirm} in
      ...
      set_pref "disk" "$disk"
      getch;;
      *) echo "Please Select your disk again";get_disk;;
    esac
